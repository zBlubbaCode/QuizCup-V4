...
public class GateCommand implements CommandExecutor {
        ...
        if(sender.hasPermission("quizcup.gates")) {
            if(args.length == 1) {
                if(args[0].equalsIgnoreCase("1")) {
                    ...
                    sender.sendMessage(MessageCollection.getPrefix() + "Gate §c#1 §7wurde aktualisiert!");
                    ...
                    sender.sendMessage(MessageCollection.getPrefix() + "Gate §a#2 §7wurde aktualisiert!");
                }
            } else sender.sendMessage(MessageCollection.getPrefix() + "§cAlias: /gate <1 | 2 | 3>");
  ...
  public void replaceMiddleGates(Material type) {
    //Change gate area
      ...
      for(int y = 800; y < 1200; y++) {
        for(int z = 30; z < 40; z++) {
          ...
          switch(world.getBlockAt(x, y, z).getType()) {
              case RED_STAINED_GLASS -> blockAbove.setType(Material.RED_STAINED_GLASS);
              case BLUE_STAINED_GLASS -> blockAbove.setType(Material.BLUE_STAINED_GLASS);
              case YELLOW_STAINED_GLASS -> blockAbove.setType(Material.YELLOW_STAINED_GLASS);
              case LIME_STAINED_GLASS -> blockAbove.setType(Material.LIME_STAINED_GLASS);
          }
        }
      }
    }
